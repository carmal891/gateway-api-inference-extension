...
=======
  ...
  if [[ "$expect" == "fail" ]]; then
    ...
    if ! echo "$out" | grep -q 'modifies migrated EPP/BBR paths'; then
      echo "FAIL: expected failure annotation"
      echo "$out"
      exit 1
    fi
    echo "ok (fails check, posts notice)"
  else
    if [[ "$status" -ne 0 ]]; then
...
>>>>>>> d349b9a4 (Add workflow_dispatch and offline test for migration notice)
    if echo "$out" | grep -q 'llm-d-migration-notice-v1'; then
...
run_case "pkg/epp change" epp fail
run_case "pkg/lwepp only" lwepp pass
>>>>>>> d349b9a4 (Add workflow_dispatch and offline test for migration notice)
